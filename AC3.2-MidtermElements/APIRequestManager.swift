...
class APIRequestManager {
    ...
    func getPrivateData(endPoint: String, callback: @escaping (Data?) -> ()) {
        ...
        let headers = [
            ...
            "authorization": "Basic a2V5LTE6dHdwTFZPdm5IbEc2ajFBZndKOWI="
        ]
        ...
        session.dataTask(with: request) { (data: Data?, urlResponse: URLResponse?, error: Error?) in
            if error != nil {
                print("Error getting data during get request")
            }
            guard let validData = data else { return }
            callback(validData)
            }.resume()
